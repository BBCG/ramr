...
\
[`library`](https://rdrr.io/r/base/library.html)`(`[`GenomicRanges`](https://bioconductor.org/packages/GenomicRanges)`)`\
...
[`library`](https://rdrr.io/r/base/library.html)`(`[`ramr`](https://github.com/BBCG/ramr)`)`\
`data``(``ramr``)`\
\
[`head`](https://rdrr.io/r/utils/head.html)`(``ramr.samples``)`\
...
[`plotAMR`](../reference/plotAMR.md)`(``data.ranges``=``ramr.data``, amr.ranges``=``ramr.tp.unique``[``1``]``)`\
...
`  `[`set.seed`](https://rdrr.io/r/base/Random.html)`(``999``)`\
\
`# unique random AMRs`\
`  ``amrs.unique`` ``<-`\
`    `[`simulateAMR`](../reference/simulateAMR.md)`(``template.ranges``=``ramr.data``, nsamples``=``25``, regions.per.sample``=``2``,`\
`                min.cpgs``=``5``, merge.window``=``1000``, dbeta``=``0.2``)`\
\
`# non-unique AMRs outside of regions with unique AMRs`\
`  ``amrs.nonunique`` ``<-`\
`    `[`simulateAMR`](../reference/simulateAMR.md)`(``template.ranges``=``ramr.data``, nsamples``=``4``, exclude.ranges``=``amrs.unique``,`\
`                sample.names``=`[`sprintf`](https://rdrr.io/r/base/sprintf.html)`(``"sample%02i"``, `[`c`](https://rdrr.io/r/base/c.html)`(``2``, ``4``, ``8``, ``16``)``)``,`\
`                samples.per.region``=``2``, min.cpgs``=``5``, merge.window``=``1000``)`\
`  `\
`# random noise outside of AMR regions`\
`  ``noise`` ``<-`\
`    `[`simulateAMR`](../reference/simulateAMR.md)`(``ramr.data``, nsamples``=``25``, regions.per.sample``=``20``,`\
`                exclude.ranges``=`[`c`](https://rdrr.io/r/base/c.html)`(``amrs.unique``, ``amrs.nonunique``)``,`\
...
`    ``found`` ``<-`` `[`getAMR`](../reference/getAMR.md)`(`\
...
`  `\
`# true positives`\
`  ``tp`` ``<-`` `[`sum`](https://rdrr.io/r/base/sum.html)`(``found`` `[`%over%`](https://rdrr.io/pkg/IRanges/man/findOverlaps-methods.html)` `[`c`](https://rdrr.io/r/base/c.html)`(``amrs.unique``, ``amrs.nonunique``)``)`\
`  `\
`# false positives`\
`  ``fp`` ``<-`` `[`sum`](https://rdrr.io/r/base/sum.html)`(``found`` `[`%outside%`](https://rdrr.io/pkg/IRanges/man/findOverlaps-methods.html)` `[`c`](https://rdrr.io/r/base/c.html)`(``amrs.unique``, ``amrs.nonunique``)``)`\
\
`# true negatives`\
`  ``tn`` ``<-`` `[`length`](https://rdrr.io/r/base/length.html)`(``all.ranges`` `[`%outside%`](https://rdrr.io/pkg/IRanges/man/findOverlaps-methods.html)` `[`c`](https://rdrr.io/r/base/c.html)`(``amrs.unique``, ``amrs.nonunique``)``)`\
`  `\
`# false negatives`\
`  ``fn`` ``<-`` `[`sum`](https://rdrr.io/r/base/sum.html)`(`[`c`](https://rdrr.io/r/base/c.html)`(``amrs.unique``, ``amrs.nonunique``)`` `[`%outside%`](https://rdrr.io/pkg/IRanges/man/findOverlaps-methods.html)` ``found``)`\
`  `\
`# accuracy, MCC`\
`  ``acc`` ``<-`` ``(``tp``+``tn``)`` ``/`` ``(``tp``+``tn``+``fp``+``fn``)`\
`  ``mcc`` ``<-`` ``(``tp``*``tn`` ``-`` ``fp``*``fn``)`` ``/`` ``(`[`sqrt`](https://rdrr.io/r/base/MathFun.html)`(``tp``+``fp``)``*`[`sqrt`](https://rdrr.io/r/base/MathFun.html)`(``tp``+``fn``)``*`[`sqrt`](https://rdrr.io/r/base/MathFun.html)`(``tn``+``fp``)``*`[`sqrt`](https://rdrr.io/r/base/MathFun.html)`(``tn``+``fn``)``)`\
`  `[`setNames`](https://rdrr.io/r/stats/setNames.html)`(`[`c`](https://rdrr.io/r/base/c.html)`(``tp``, ``fp``, ``tn``, ``fn``)``, `[`c`](https://rdrr.io/r/base/c.html)`(``"TP"``, ``"FP"``, ``"TN"``, ``"FN"``)``)`\
`#>  TP  FP  TN  FN `\
`#>  58   0 206   0`\
`  `[`setNames`](https://rdrr.io/r/stats/setNames.html)`(`[`c`](https://rdrr.io/r/base/c.html)`(``acc``, ``mcc``)``, `[`c`](https://rdrr.io/r/base/c.html)`(``"accuracy"``, ``"MCC"``)``)`\
`#> accuracy      MCC `\
`#>        1        1`
...
\
`# identify AMRs`\
`amrs`` ``<-`` `[`getAMR`](../reference/getAMR.md)`(`\
...
\
`# inspect`\
[`sort`](https://rdrr.io/pkg/BiocGenerics/man/sort.html)`(``amrs``)`\
...
[`library`](https://rdrr.io/r/base/library.html)`(``annotatr``)`\
`annotation.types`` ``<-`` `[`c`](https://rdrr.io/r/base/c.html)`(``"hg19_cpg_inter"``, ``"hg19_cpg_islands"``, ``"hg19_cpg_shores"``,`\
`                      ``"hg19_cpg_shelves"``, ``"hg19_genes_intergenic"``, ``"hg19_genes_promoters"``,`\
`                      ``"hg19_genes_5UTRs"``, ``"hg19_genes_firstexons"``, ``"hg19_genes_3UTRs"``)`\
`annotations`` ``<-`` `[`build_annotations`](https://rdrr.io/pkg/annotatr/man/build_annotations.html)`(``genome``=``'hg19'``, annotations``=``annotation.types``)`\
...
`amrs.annots`` ``<-`` `[`annotate_regions`](https://rdrr.io/pkg/annotatr/man/annotate_regions.html)`(``regions``=``amrs``, annotations``=``annotations``, ignore.strand``=``TRUE``, quiet``=``FALSE``)`\
`#> Annotating...`\
[`summarize_annotations`](https://rdrr.io/pkg/annotatr/man/summarize_annotations.html)`(``annotated_regions``=``amrs.annots``, quiet``=``FALSE``)`\
...
`#>  [1] org.Hs.eg.db_3.23.1                      TxDb.Hsapiens.UCSC.hg19.knownGene_3.22.1`\
`#>  [3] GenomicFeatures_1.65.0                   AnnotationDbi_1.75.2                    `\
`#>  [5] Biobase_2.73.2                           annotatr_1.39.0                         `\
`#>  [7] gridExtra_2.3.1                          ramr_1.21.3                             `\
`#>  [9] ggplot2_4.0.3                            GenomicRanges_1.65.4                    `\
...
`#>  [7] matrixStats_1.5.0        compiler_4.6.1           RSQLite_3.53.3          `\
`#> [10] png_0.1-9                systemfonts_1.3.2        vctrs_0.7.3             `\
`#> [13] reshape2_1.4.5           stringr_1.6.0            pkgconfig_2.0.3         `\
`#> [16] crayon_1.5.3             fastmap_1.2.0            dbplyr_2.6.0            `\
`#> [19] XVector_0.53.0           labeling_0.4.3           utf8_1.2.6              `\
`#> [22] Rsamtools_2.29.0         rmarkdown_2.32           tzdb_0.5.0              `\
`#> [25] UCSC.utils_1.9.0         ragg_1.5.2               purrr_1.2.2             `\
`#> [28] bit_4.6.0                xfun_0.60                cachem_1.1.0            `\
`#> [31] cigarillo_1.3.1          GenomeInfoDb_1.49.1      jsonlite_2.0.0          `\
`#> [34] blob_1.3.0               BiocParallel_1.47.0      parallel_4.6.1          `\
`#> [37] R6_2.6.1                 bslib_0.12.0             stringi_1.8.9           `\
`#> [40] RColorBrewer_1.1-3       rtracklayer_1.73.0       jquerylib_0.1.4         `\
`#> [43] Rcpp_1.1.2               knitr_1.52               readr_2.2.0             `\
`#> [46] BiocBaseUtils_1.15.1     Matrix_1.7-6             tidyselect_1.2.1        `\
`#> [49] yaml_2.3.12              codetools_0.2-20         curl_8.0.0              `\
`#> [52] lattice_0.23-1           tibble_3.3.1             regioneR_1.45.0         `\
`#> [55] plyr_1.8.9               withr_3.0.3              KEGGREST_1.53.6         `\
`#> [58] S7_0.2.2                 evaluate_1.0.5           desc_1.4.3              `\
`#> [61] BiocFileCache_3.3.0      Biostrings_2.81.9        pillar_1.11.1           `\
`#> [64] BiocManager_1.30.27      filelock_1.0.3           RCurl_1.98-1.20         `\
`#> [67] BiocVersion_3.24.0       hms_1.1.4                scales_1.4.0            `\
`#> [70] glue_1.8.1               tools_4.6.1              AnnotationHub_4.3.2     `\
`#> [73] BiocIO_1.23.3            data.table_1.18.6.1      BSgenome_1.81.1         `\
`#> [76] GenomicAlignments_1.49.2 fs_2.1.0                 XML_3.99-0.24           `\
`#> [79] grid_4.6.1               restfulr_0.0.17          cli_3.6.6               `\
`#> [82] rappdirs_0.3.4           textshaping_1.0.5        dplyr_1.2.1             `\
`#> [85] gtable_0.3.6             sass_0.4.10              digest_0.6.39           `\
`#> [88] rjson_0.2.23             htmlwidgets_1.6.4        farver_2.1.2            `\
`#> [91] memoise_2.0.1            htmltools_0.5.9          pkgdown_2.2.1.9000      `\
`#> [94] lifecycle_1.0.5          httr_1.4.9               bit64_4.8.6`
